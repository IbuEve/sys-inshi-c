...
void search_btree(PTree t, int val, Node *r_node, int *r_i){
    ...
    while(p != NULL) {
        ...
        if (i < p->key_num && val == p->key[i]) {r_node = p; *r_i = i; return; } // 【1】val == p->key[i]だけだと、whileが i < p->key_numで終了した場合に、iが配列の範囲外を指してしまうからエラーになる
        else { p = p->child[i]; }
    }
    r_node = NULL; *r_i = -1; return; // 【2】
}
...
int main(){
    ...
    search_btree(t, 7, r_nd, &r_id); printf("%d\n", r_id);
    return 0;
}